...
void sortTeachersBySalary(MyContainer<Teacher>& staff) {
    // std::sort вызывается, но перегрузка с концептом IsSortedIt обнаруживает,
    // что контейнер уже отсортирован, и не выполняет лишнюю работу.
    std::sort(staff.begin(), staff.end(),
              [](const Teacher& a, const Teacher& b) {
                  return a.getSalary() < b.getSalary();
              });

    std::cout << "\n--- Список учителей после std::sort (контейнер уже отсортирован) ---\n";
